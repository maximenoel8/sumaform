    ...
    content = yamlencode(merge(
      {
        ...
        server_username               = var.grains["product_version"] == "paygo" && contains(var.roles, "server") && length(aws_instance.instance) > 0 ? aws_instance.instance[*].id[0] : var.grains["server_username"]
      },
    var.grains))
